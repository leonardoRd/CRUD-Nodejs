...
import {
  getTipoComprobRequest,
  getTiposComprobRequest,
  createTipoComprobRequest,
  deleteTipoComprobRequest,
  updateTipoComprobRequest,
} from '../api/tipoComprobante'
...
export const TipoComprobanteProvider = ({ children }) => {
  ...
  const deleteTipoComprob = async (id) => {
    try {
      ...
      if (res.status === 200)
        setTipoComprob(tipoComprob.filter((tipo) => tipo._id != id))
    } catch (error) {
      console.error(error)
    }
  }
